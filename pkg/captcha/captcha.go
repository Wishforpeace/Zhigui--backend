...
// once 确保 internalCaptcha 对象只能初始化一次
var once sync.Once
...
func NewCaptcha() *Captcha {
	once.Do(func() {
		...
		store := RedisStrore{
			RedisClinet: redis.Redis,
			KeyPrefix:   config.GetString("web.name") + ":captcha",
		}
		// 配置 base64Captcha 驱动信息
		driver := base64Captcha.NewDriverDigit(
			config.GetInt("captcha.height"),      // 宽
			config.GetInt("captcha.width"),       // 高
			config.GetInt("captcha.length"),      // 长
...
func (c *Captcha) VerifyCaptcha(id string, answer string) (match bool) {
	...
	// 方便本地和 API 自动测试
	if !web.IsProduction() && id == config.GetString("captcha.testing_key") {
		return true
	}
